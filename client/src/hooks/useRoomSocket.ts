...
export function useRoomSocket() {
  ...
  return {
    createRoom: () => {
      const sent = socket.emit('room:create', { nickname: nickname.trim() })
      return sent ? { ok: true } : { ok: false, reason: '网络未连接，发送失败' }
    },
    joinRoom: () => {
      const sent = socket.emit('room:join', {
    ...
    leaveRoom: () => {
      if (!currentRoom) {
        return { ok: false, reason: '当前不在房间内' }
      }

      const sent = socket.emit('room:leave', {
    ...
    toggleReady: (isReady: boolean) => {
      ...
      const sent = socket.emit('room:ready', {
        roomId: currentRoom.roomId,
        isReady,
      })
      return sent ? { ok: true } : { ok: false, reason: '网络未连接，发送失败' }
